...
## Table 2. Vaccine-Related Questions
...
# Build table
table_2 <- map_dfr(vaccine_vars, function(var) {
  ...
  # Special handling for JHU7_9
  if (var == "JHU7_9") {
    topic <- "<strong>Which issues, if any, mattered to you during the 2024 presidential election? Choose up to 3.</strong>"
  } else if (var == "JHU5A") {
    topic <- "<strong>Here is a list of specific policy goals. To what extent would you support or oppose them?</strong><br><em>Continue government support for research and development of new vaccines</em>"
  } else if (var == "JHU6C") {
    topic <- "<em>Removing school vaccination requirements</em>"
  } else {
  ...
  # For JHU7_9, only keep "Yes" but change the label
  if (var == "JHU7_9") {
    responses <- "<em>Vaccines</em> [selected as 1 of 3]"
  }
  ...
  category_rows <- map_dfr(responses, function(response) {
    temp_data <- data |> 
      ...
      mutate(temp = ifelse(
        # Special handling for JHU7_9 to look for "Yes" responses
        if (var == "JHU7_9") {
          as.character(get(var)) == "Yes"
        } else {
          as.character(get(var)) == response
        },
        1, 0))
