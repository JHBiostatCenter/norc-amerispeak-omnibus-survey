...
create_ed_vaccine_crosstab <- function(var, question_text) {
  ...
  response_rows <- map_dfr(responses, function(response) {
    ...
    row_data <- tibble(Response = response)
    for (ed in ed_levels) {
...
table_2_1 <- create_ed_vaccine_crosstab(
  "JHU5A",
  "<strong>Continue government support for research and development of new vaccines</strong>"
)

table_2_2 <- create_ed_vaccine_crosstab(
  "JHU6C",
  "<strong>Removing school vaccination requirements</strong>"
)

# Combine tables
table_2_combined <- bind_rows(table_2_1, table_2_2)
